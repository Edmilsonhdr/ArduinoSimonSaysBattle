...

{
...

    {
        ...
        while (checkButton() != CHOICE_NONE)
            ; // Aguarde até que o usuário pare de pressionar o botão
...

{
...

    {
...

        {
            ...
            if (choice == 0)
                return false; // Se a espera expirar, o jogador perde

            if (choice != gameBoard[currentMove])
                return false; // Se a escolha estiver incorreta, o jogador perde
        }
...

{
...

    {
...

        {
            ...
            while (checkButton() != CHOICE_NONE)
                ; // Agora vamos esperar que o usuário libere o botão
...

{
    ...
    setLEDs(1 << LEDnumber); // Mud301e o LED

    LEDnumber++; // Ir para o próximo LED

    if (LEDnumber > 3)
        LEDnumber = 0; // Enrole o balcão, se necessário
}